...
export class SheetDataBuilder {
    ...
    addIssues(issues: Array<Issue>): SheetDataBuilder {
        for (let index = 0; index < issues.length; index++) {
            ...
            const dateCell = this.data[0].find(x => x.value === issue.created.toLocaleDateString());
            const userCell = this.data.map(x => x[index]).find(x => x.value === issue.assignee.displayName);
